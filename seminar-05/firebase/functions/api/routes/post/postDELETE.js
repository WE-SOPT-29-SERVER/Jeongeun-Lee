...
//   Number(req.user.id)
  ...
  try {
    ...
    const deletedPost = await postDB.deletePost(client, postId, Number(req.user.id));
    if (!deletedPost) return res.status(statusCode.NOT_FOUND).send(util.fail(statusCode.NOT_FOUND, responseMessage.NO_POST));

    res.status(statusCode.OK).send(util.success(statusCode.OK, responseMessage.POST_DELETE_SUCCESS, deletedPost));
  } catch (error) {
